...
/*--- Struct Definitions ---*/
extern int push_arg; // External variable to hold argument for push operation

/**
 * struct stack_s - Doubly linked list node structure for stack or queue
 * @n: integer value stored in the node
 * @prev: pointer to the previous element in the stack or queue
 * @next: pointer to the next element in the stack or queue
 * Description: Node structure used for implementing stack or queue
 * for Holberton project following LIFO and FIFO principles
 */
...
/**
 * struct instruction_s - Structure for opcode and its corresponding function
 * @opcode: the operation code (command)
 * @f: function to execute the command
 * Description: Structure that pairs an opcode with its function
...
extern global_var var_global; // Extern declaration of global variable structure
...
void _pchar(stack_t **stack, unsigned int line_number); // Print the char at the top of the stack
void _pstr(stack_t **stack, unsigned int line_number);  // Print the string starting from the top of the stack

/* Mathematical operation functions */
void _sub(stack_t **stack, unsigned int line_number); // Subtract the top element from the second top element
